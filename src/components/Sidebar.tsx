  ...
  return (
    <aside className="w-[240px] p-[30px_40px] fixed h-screen overflow-y-auto top-5 left-[15px] flex flex-col flex-shrink-0 hidden md:flex" id="sidebar">
      ...
      <nav className="flex flex-col gap-[15px] mb-10">
        <Link href="/about" className="text-black no-underline text-base cursor-pointer transition-opacity duration-200 ease-in-out hover:opacity-50">
          About
        </Link>
        <Link href="/stories" className="text-black no-underline text-base cursor-pointer transition-opacity duration-200 ease-in-out hover:opacity-50">
          Stories
        </Link>

        {/* About Preview Section - Only show when not on About or Stories page */}
        {!isAboutPage && !isStoriesPage && (
          <div className="mt-2 mb-5">
            <Link href="/about" className="text-sm leading-[1.4] mb-0 text-[#333] no-underline cursor-pointer transition-opacity duration-200 ease-in-out hover:opacity-70 block">
              Hi, I&apos;m Mattia! I run an investments firm between San Francisco and New York and document the world&apos;s most extreme rituals.
